...
```shell-session
[!bash!]$ sudo ip addr add 192.168.1.1/24 dev eth0.20
[!bash!]$ sudo ip link set up eth0.20
```

At last, we can check whether the interface has changed states to up:

```shell-session
[!bash!]$ ip a | grep eth0.20
